...
GitHub Actions로 하루 2회 자동 수집 및 배포됩니다.

| 시간 (KST) | 작업 | 내용 |
|-------------|------|------|
| **08:00** | 아침 뉴스 수집 | 21개 산업 뉴스 크롤링 → JSON 갱신 |
| **16:30** | 장 마감 전체 업데이트 | ETF/주식/지수/뉴스 전체 수집 → JSON 갱신 |

수집 완료 시 `data/`와 `public/data/dashboard.json`이 자동 커밋되고, Vercel이 재배포합니다.
